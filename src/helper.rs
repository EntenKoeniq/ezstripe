pub async fn post_request<T>(request: reqwest::RequestBuilder) -> Result<T, (String, Option<crate::error::Info>)> where T : serde::de::DeserializeOwned {
...
pub async fn get_request<T>(request: reqwest::RequestBuilder) -> Result<T, (String, Option<crate::error::Info>)> where T : serde::de::DeserializeOwned {
  let response = match request.send().await {
      Ok(r) => r,
      Err(_) => return Err(("Request failed".to_string(), None))
    };
